    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                // Accept a single cart ID or an array of cart IDs
                'cart_id'       => 'required',
            ...
            // Load the cart with its cartItems and each cartItem's product.
            $firstCartId = $cartIds[0];
            $cart = Cart::with('cartItems.product')->find($firstCartId);

            $productImage = null;
            $description  = "Your order has been placed.";

            if ($cart && $cart->cartItems->isNotEmpty()) {
                $product = $cart->cartItems->first()->product;
                if ($product) {
